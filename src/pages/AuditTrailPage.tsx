...
export default function AuditTrailPage() {
  ...
  const columns: Column<AuditLogRow>[] = [
    { key: "createdAt", header: t("When"), render: (r) => <span className="text-xs text-ink-muted tabular-nums">{formatDateTime(r.createdAt)}</span> },
    {
      key: "actor",
      header: t("Who"),
      render: (r) => (r.actor ? <span className="text-ink">{r.actor.name}</span> : <span className="text-ink-muted">{t("System")}</span>),
    },
    { key: "action", header: t("Action"), render: (r) => <code className="rounded bg-plane px-1.5 py-0.5 text-xs text-ink-secondary">{r.action}</code> },
    { key: "entityType", header: t("Entity"), render: (r) => r.entityType },
    { key: "entityId", header: t("Entity ID"), render: (r) => <span className="text-xs text-ink-muted">{r.entityId ?? "—"}</span> },
    { key: "ipAddress", header: "IP", render: (r) => <span className="text-xs text-ink-muted">{r.ipAddress ?? "—"}</span> },
  ];

  return (
    <>
      <PageHeader title={t("Audit Trail")} description={t("Every recorded action in your tenant, who did it, and when - your own copy, self-serve (Pro+).")} />
      ...
      <Card>
        ...
        {list.isLoading ? (
          ...
          <EmptyState icon={<History className="h-5 w-5" />} title={t("No audit events found")} description={t("Try widening your date range or clearing the entity filter.")} />
        ) : (
