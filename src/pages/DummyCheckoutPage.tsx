    ...
    <div className="flex min-h-screen flex-col items-center justify-center gap-6 bg-plane px-6 py-12">
      <div className="w-full max-w-sm rounded-card border border-border bg-surface-raised p-6 shadow-card">
        <div className="mb-1 flex items-center gap-2 text-[11px] font-mono uppercase tracking-wide text-ink-muted">
          ...
          {t("Dummy checkout simulator")}
        </div>
        <p className="mb-6 text-[11px] text-ink-muted">
          {t("No live Fiuu credentials are configured, so this stands in for the real payment page. Set")} <code className="font-mono">FIUU_MERCHANT_ID</code>/<code className="font-mono">FIUU_VERIFY_KEY</code> {t("to use real Fiuu checkout instead.")}
        </p>
        ...
        <p className="mt-1 font-mono text-[11px] text-ink-muted">{t("Order")} {orderId}</p>
