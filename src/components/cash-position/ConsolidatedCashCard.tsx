...
export function ConsolidatedCashCard() {
  return (
    <PlanGate module="advanced_insights" feature="Consolidated Cash Position">
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-1.5">
          <Globe className="h-4 w-4 text-ink-muted" /> Consolidated Total ({data?.baseCurrency ?? "..."})
        </CardTitle>
      </CardHeader>
      <CardBody>
        {isLoading || !data ? (
          ...
          <>
            ...
            <p className="mt-1 text-xs text-ink-muted">Live FX rates (daily reference rates, not intraday trading rates)</p>
            <div className="mt-3 space-y-1.5">
              {data.breakdown.map((b) => (
                <div key={b.currencyCode} className="flex items-center justify-between text-[12.5px]">
                  ...
                  <span className="tabular-nums font-medium text-ink">{b.totalInBase !== null ? formatMoney(b.totalInBase, data.baseCurrency) : "rate unavailable"}</span>
                </div>
              ))}
            </div>
            {data.unconvertedCurrencies.length > 0 && (
              <p className="mt-2 text-[11px] text-status-warning">Could not fetch a rate for: {data.unconvertedCurrencies.join(", ")} - excluded from the total above.</p>
            )}
          </>
        )}
      </CardBody>
