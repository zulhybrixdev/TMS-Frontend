...
export function AccountDetailDrawer({ account, onClose, onUpdated }: Props) {
  ...
  const saveSettings = settingsForm.handleSubmit(async (values) => {
    try {
      ...
      toast.success(t("Account settings updated"));
      onUpdated(updated);
    } catch (err) {
      toast.error(t("Could not update account"), { description: err instanceof ApiError ? err.message : undefined });
    }
  });
  ...
  const saveBalance = balanceForm.handleSubmit(async (values) => {
    try {
      ...
      toast.success(t("Balance recorded"), {
        description: values.balanceDate === todayLocal() ? t("Current balance and cash position have been updated.") : t("Recorded in that day's history - the current balance is unchanged."),
      });
      onUpdated(updated);
    } catch (err) {
      toast.error(t("Could not record balance"), { description: err instanceof ApiError ? err.message : undefined });
    }
  });
    ...
    <Dialog open onClose={onClose} title={account.accountName} description={`${account.bankName} · ${account.accountNumber}`} size="lg">
      <Tabs
        tabs={[
          ...
          ...(canManage ? [{ key: "settings", label: t("Settings") }, { key: "balance", label: t("Record Balance") }] : []),
        ]}
        active={tab}
        onChange={setTab}
      />
      ...
      {tab === "overview" && (
        <div className="pt-4">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <Metric label={t("Current")} value={formatMoney(account.currentBalance, account.currencyCode)} />
            <Metric label={t("Available")} value={formatMoney(account.availableCash, account.currencyCode)} />
            <Metric label={t("Minimum")} value={formatMoney(account.minimumBalance, account.currencyCode)} />
            <Metric label={t("Target")} value={formatMoney(account.targetBalance, account.currencyCode)} />
          </div>
          {(account.overdraftLimit > 0 || account.floatTotal > 0 || account.siteName) && (
            <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {account.overdraftLimit > 0 && (
                <>
                  <Metric label={t("Overdraft Limit")} value={formatMoney(account.overdraftLimit, account.currencyCode)} />
                  <Metric label={t("Overdraft Used")} value={formatMoney(account.overdraftUtilised, account.currencyCode)} tone={account.overdraftUtilised > 0 ? "warning" : undefined} />
                  <Metric label={t("Overdraft Left")} value={formatMoney(account.overdraftAvailable, account.currencyCode)} />
                  <Metric label={t("Available incl. OD")} value={formatMoney(account.liquidity, account.currencyCode)} />
                </>
              )}
              {account.floatTotal > 0 && (
                <>
                  <Metric label={t("Day 1 Float")} value={formatMoney(account.floatDay1, account.currencyCode)} />
                  <Metric label={t("Day 2 Float")} value={formatMoney(account.floatDay2, account.currencyCode)} />
                </>
              )}
              {account.siteName && <Metric label={t("Site")} value={account.siteName} />}
            </div>
          )}
          <div className="mt-3 flex items-center gap-2">
            <StatusBadge status={account.cashStatus} />
            <StatusBadge status={account.status} />
            {account.shortfall > 0 && <span className="text-xs text-status-critical">{t("Shortfall of")} {formatMoney(account.shortfall, account.currencyCode)}</span>}
            {account.excessCash > 0 && <span className="text-xs text-brand">{t("Excess of")} {formatMoney(account.excessCash, account.currencyCode)}</span>}
          </div>

          <p className="mb-2 mt-6 text-[13px] font-medium text-ink-secondary">{t("Balance History · 60 Days")}</p>
          {historyLoading ? (
            <Skeleton className="h-52 w-full" />
          ) : !history || history.length === 0 ? (
            <p className="py-8 text-center text-[13px] text-ink-muted">{t("No balance history recorded yet.")}</p>
          ) : (
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={history.map((h) => ({ date: h.balanceDate, closingBalance: Number(h.closingBalance) }))}>
                ...
                <Area type="monotone" dataKey="closingBalance" stroke="var(--series-1)" strokeWidth={2} fill="url(#acctFill)" name={t("Balance")} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      {tab === "settings" && canManage && (
        <form className="space-y-4 pt-4" onSubmit={saveSettings}>
          <p className="text-[13px] text-ink-secondary">{t("Configure the minimum and target balance thresholds used by the cash engine's shortfall/excess recommendations.")}</p>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label htmlFor="minimumBalance">{t("Minimum Balance")}</Label>
              <Input id="minimumBalance" type="number" step="0.01" {...settingsForm.register("minimumBalance")} />
            </div>
            <div>
              <Label htmlFor="targetBalance">{t("Target Balance")}</Label>
              <Input id="targetBalance" type="number" step="0.01" {...settingsForm.register("targetBalance")} />
            </div>
            <div>
              <Label htmlFor="reservedAmount">{t("Reserved Amount")}</Label>
            ...
            <div>
              ...
              <Input id="siteName" list="drawer-site-options" placeholder={t("e.g. PJRM, Bukit Raja")} {...settingsForm.register("siteName")} />
      ...
      {tab === "balance" && canManage && (
        <form className="space-y-4 pt-4" onSubmit={saveBalance}>
          <p className="text-[13px] text-ink-secondary">
            {t("Record a closing balance from the bank statement. Today's date updates the current balance and cash position immediately; an earlier date only corrects that day's history and leaves the current balance alone. The balance can be negative for an account in overdraft.")}
          </p>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label htmlFor="balanceDate">{t("Balance Date")}</Label>
              <Input id="balanceDate" type="date" {...balanceForm.register("balanceDate")} />
            </div>
            <div>
              <Label htmlFor="closingBalance">{t("Closing Balance")}</Label>
