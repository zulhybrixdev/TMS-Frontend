      ...
      <div className="flex items-center justify-between rounded-lg bg-plane px-4 py-3">
        <div>
          <p className="text-[13px] font-medium text-ink">
            {t("Approval Level {current} of {total}", { current: request.currentLevel, total: request.requiredLevels })}
          </p>
          <p className="text-xs text-ink-muted">{t("Status:")} {t(request.status.replace(/_/g, " "))}</p>
        </div>
      </div>
      ...
      <ol className="space-y-3">
        {request.actions.length === 0 && (
          <li className="flex items-center gap-2.5 text-[13px] text-ink-muted">
            <Clock className="h-4 w-4" /> {t("Awaiting first approval action")}
          </li>
        )}
          ...
          return (
            <li key={action.id} className="flex gap-3">
              ...
              <div className="min-w-0">
                <p className="text-[13px] font-medium text-ink">
                  {action.actor.name} <span className="font-normal text-ink-secondary">{action.action === "APPROVE" ? t("· Level {n} approved", { n: action.level }) : action.action === "REJECT" ? t("· Level {n} rejected", { n: action.level }) : t("· Level {n} commented", { n: action.level })}</span>
                </p>
