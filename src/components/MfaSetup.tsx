...
export function MfaSetupDialog({ onClose, onEnabled }: { onClose: () => void; onEnabled: (result: MfaEnableResult) => void }) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) => {
        setError(err instanceof ApiError ? err.message : t("Could not start MFA setup"));
        setStep("error");
      });
  }, []);
  ...
  return (
    <Dialog open onClose={onClose} title={t("Set up Two-Factor Authentication")} size="md">
      {step === "loading" && <p className="text-[13px] text-ink-secondary">{t("Generating your setup key...")}</p>}
      {step === "error" && <p className="text-[13px] text-status-critical">{error}</p>}
      {step === "scan" && (
        <div className="space-y-4">
          <p className="text-[13px] text-ink-secondary">{t("Scan this QR code with your authenticator app, then enter the 6-digit code it shows.")}</p>
...
export function RecoveryCodesDialog({ codes, onClose }: { codes: string[]; onClose: () => void }) {
  ...
  return (
    <Dialog open onClose={onClose} title={t("Save your recovery codes")} size="md">
      <div className="space-y-4">
        <p className="text-[13px] text-ink-secondary">
          {t("Two-factor authentication is now enabled. Store these one-time recovery codes somewhere safe - each one can be used once to sign in if you lose access to your authenticator app. They won't be shown again.")}
        </p>
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={copyAll} className="flex-1 justify-center">
            <Copy className="h-4 w-4" /> {t("Copy all")}
          </Button>
