...
export function QuotaPanel({ quotas, onManage }: { quotas: QuotaUsage[]; onManage?: () => void }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Released Quota</CardTitle>
        {onManage && (
          <Button size="sm" variant="outline" onClick={onManage}>
            <Settings2 className="h-3.5 w-3.5" /> Set quotas
          </Button>
        )}
      </CardHeader>
      <CardBody className="space-y-5">
        {quotas.length === 0 ? (
          <p className="text-[13px] text-ink-muted">No cheque or bank draft quota is set up yet.{onManage ? " Use “Set quotas” to add one, e.g. the MBSB bank draft limit." : ""}</p>
        ) : (
    ...
    <div>
      <div className="flex items-center justify-between gap-2">
        <p className="text-[13.5px] font-medium text-ink">
          ...
          {METHOD_LABEL[q.paymentMethod]}
        </p>
        {q.exceeded ? <Badge tone="critical">Over quota</Badge> : q.utilisationPct >= 80 ? <Badge tone="warning">{q.utilisationPct}% used</Badge> : <Badge tone="neutral">{q.utilisationPct}% used</Badge>}
      </div>
      ...
      <dl className="mt-2 grid grid-cols-3 gap-2 text-xs">
        ...
        <div>
          <dt className="text-ink-muted">Remaining of {fmt(limit)}</dt>
      ...
      {usesAmount && q.dailyCountLimit !== null && (
        <p className="mt-1.5 text-xs text-ink-muted">
          Count: {q.releasedCount + q.pendingCount} of {q.dailyCountLimit} ({q.releasedCount} released)
        </p>
      )}
    </div>
  );
